...
def thermo_statis(dataframe_log):
    ...
    volume = np.mean(dataframe_log.loc[40000:, "Volume"]) * CONSTANTS.angstrom**3
...
def cal_ep(dipole_x_array, dipole_y_array, dipole_z_array, vol, temp):
    m_2 = (
        np.sum(
            np.array(
                [
                    np.var(dipole_x_array, axis=0),
                    np.var(dipole_y_array, axis=0),
                    np.var(dipole_z_array, axis=0),
                ]
            )
        )
