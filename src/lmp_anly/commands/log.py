...
from lmp_anly.cal_ep_via_log import MissingValueError, cal_ep_via_log
...
def log(
    file: Annotated[str, typer.Argument(help="relative path of LAMMPS log file")],
    figformat: Annotated[
        Literal["png", "svg"],
    ...
    epsilon: Annotated[
        bool,
        typer.Option(
            "--epsilon", "-e", help="caculate epsilon via dipole moment in log"
        ),
    ] = False,
):
    ...
    if epsilon:
        try:
            ...
            cal_ep_via_log(
                df_log=df_log, line_element=line_element, epsilon_dump_file=epsilon_file
            )
        except MissingValueError as e:
            print(
                f"[green bold]{e.missing_keys}[/green bold] not in LAMMPS log \nLAMMPS log head is \n{e.context['log_head']}"
            )
